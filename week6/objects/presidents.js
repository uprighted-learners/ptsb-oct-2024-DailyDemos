...
// a. Given an Array of US Presidents
// b. Complete the following tasks
// i. console.log the number of Presidents we’ve had
console.log(presidents.length);
// ii. Create a list of all Presidents that are alive
console.log(
  presidents
    .filter((x) => x.passed === undefined)
    .map((x) => `${x.first} ${x.last}`)
);
// iii. Create a list of all Presidents that have passed away
console.log(
  presidents
    .filter((x) => x.passed !== undefined)
    .map((x) => x.first + " " + x.last)
);
// iv. Use forEach() to log “first name, last name, & was XX years old when
// they died.” to the console.
// v. For every living President calculate their current age
